...
KEY_ISR:
                ...
                LDR      R4, =0xFF200050        // R4 holds address of KEY
                LDR      R5, [R4, #0x0C]        // R5 <- KEY Edgecapture
                LDR      R3, =0xFF200020        // R3 <- Address of HEX3-0
                ...
                STR      R5, [R4, #0x0C]        // Reset Edgecapture
                
                POP      {R0-R7, LR}
                MOV      PC, LR

ZERO:           LDRB     R0, [R3]               // Check HEX0
                CMP      R0, #0                 
                MOVEQ    R1, #0b00111111        // Not lit up -> draw "0"
                MOVNE    R1, #0                 // Being lit up --> clear
                STRB     R1, [R3]
                MOV      PC, LR

ONE:            LDRB     R0, [R3, #1]           // Check HEX1
                CMP      R0, #0                 
                MOVEQ    R1, #0b00000110        // Not lit up -> draw "1"
                MOVNE    R1, #0                 // Being lit up --> clear
                STRB     R1, [R3, #1]
                MOV      PC, LR

TWO:            LDRB     R0, [R3, #2]           // Check HEX2
                CMP      R0, #0                 
                MOVEQ    R1, #0b01011011        // Not lit up -> draw "2"
                MOVNE    R1, #0                 // Being lit up --> clear
                STRB     R1, [R3, #2]
                MOV      PC, LR

THREE:          LDRB     R0, [R3, #3]           // Check HEX3
                CMP      R0, #0                 
                MOVEQ    R1, #0b01001111        // Not lit up -> draw "3"
                MOVNE    R1, #0                 // Being lit up --> clear
                STRB     R1, [R3, #3]
                MOV      PC, LR
